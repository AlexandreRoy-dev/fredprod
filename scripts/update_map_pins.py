...
USER_AGENT = "FredRoySiteMapPins/1.0 (fredroy.ca)"

AREA_CITY = {
    "Mirabel": "Mirabel, Québec",
    "Domaine Vert Nord": "Mirabel, Québec",
    "Saint-Jérôme": "Saint-Jérôme, Québec",
    "Saint-Antoine": "Saint-Jérôme, Québec",
    "Saint-Sauveur": "Saint-Sauveur, Québec",
    "Saint-Colomban": "Saint-Colomban, Québec",
    "Saint-Lin": "Saint-Lin-Laurentides, Québec",
    "Saint-Lin/Laurentides": "Saint-Lin-Laurentides, Québec",
    "Blainville": "Blainville, Québec",
    "Nord": "Blainville, Québec",
    "Sud": "Saint-Joseph-du-Lac, Québec",
    "Saint-Antoine": "Saint-Jérôme, Québec",
    "Rosemère": "Rosemère, Québec",
    "Prévost": "Prévost, Québec",
    "Sainte-Catherine": "Sainte-Catherine, Québec",
    "Brownsburg-Chatham": "Brownsburg-Chatham, Québec",
    "Lac-Supérieur": "Lac-Supérieur, Québec",
    "Saint-Jean-de-Matha": "Saint-Jean-de-Matha, Québec",
    "Saint-Joseph-du-Lac": "Saint-Joseph-du-Lac, Québec",
    "Sud": "Saint-Joseph-du-Lac, Québec",
    "Mascouche": "Mascouche, Québec",
    "Albi": "Mascouche, Québec",
    "Bois-des-Filion": "Bois-des-Filion, Québec",
    "Saint-Hippolyte": "Saint-Hippolyte, Québec",
    "Laval": "Laval, Québec",
    "Terrebonne": "Terrebonne, Québec",
...
def street_name_only(address: str) -> str:
    ...
    for part in parts:
        ...
        if re.fullmatch(r"(?:app|apt|unit|suite|#)\s*\.?\s*\d+\w*", part, re.I):
            continue
        # Keep ordinal street names: 36e Avenue, 1re Rue, 2e Avenue
        if re.match(r"^\d+(?:e|er|ère|re|nd|rd|th)\b", part, re.I):
            kept.append(part)
            continue
...
MAP_SECTION_HTML = """
    <section id="transactions-map-section" class="py-24 md:py-32 bg-fred-gray relative overflow-hidden">
      <div class="relative z-10 max-w-7xl mx-auto px-6 md:px-12">
        <div class="grid lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          <div class="lg:col-span-4">
            <span class="text-fred-gold uppercase tracking-luxury text-xs font-bold block mb-4">Portfolio</span>
            <h2 class="text-4xl md:text-5xl font-bold text-fred-blue leading-tight mb-6">
              Transactions réussies
            </h2>
            <p class="text-gray-500 font-light text-lg leading-relaxed mb-10">
              Explorez mes transactions dans les Laurentides, Laval et la Rive-Nord. Chaque point représente un projet accompli (noms de rues seulement).
            </p>
            <a href="proprietes.html" class="inline-block bg-fred-blue text-white px-10 py-4 text-xs font-bold uppercase tracking-luxury hover:bg-fred-gold transition-colors duration-300 shadow-xl">
              Voir mes propriétés
            </a>
          </div>
          <div class="lg:col-span-8">
            <div class="w-full h-[420px] md:h-[520px] overflow-hidden shadow-2xl border border-fred-gold/30 bg-white">
...
MAP_HEAD_LINKS = """
  ...
  <style>
    .marker-cluster-small, .marker-cluster-medium, .marker-cluster-large { background-color: rgba(40,46,100,0.25) !important; }
    .marker-cluster-small div, .marker-cluster-medium div, .marker-cluster-large div {
      background-color: #282e64 !important; color: #fff !important; font-weight: 700;
    }
    #transactionsMap .leaflet-popup-content-wrapper { border-radius: 4px; }
  </style>
"""
    ...
    fetch('data/transaction_pins.json')
      ...
      .then(function(data) {
        ...
        properties.forEach(function(prop) {
          ...
          var marker = L.marker([prop.lat, prop.lng], {icon: brandIcon})
            .bindPopup(
              '<div style="text-align:center;font-family:Montserrat,sans-serif;min-width:140px;">'
              + '<div style="color:#be9018;text-transform:uppercase;font-weight:700;font-size:10px;letter-spacing:1px;margin-bottom:4px;">' + (prop.area || '') + '</div>'
              + '<div style="font-weight:700;color:#282e64;font-size:14px;margin-bottom:8px;">' + (prop.title || '') + '</div>'
              + '<div style="display:inline-block;background:#282e64;color:#fff;padding:6px 10px;font-size:11px;font-weight:700;letter-spacing:0.08em;">' + badge + '</div>'
              + '</div>'
            );
